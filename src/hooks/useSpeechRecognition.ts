...
import { buildSessionTranscript } from '../utils/appendVoiceChunk'
  ...
  const attachHandlers = useCallback(
    (recognition: SpeechRecognitionInstance) => {
      recognition.onresult = (event) => {
        ...
        const slices = Array.from({ length: event.results.length }, (_, index) => ({
          isFinal: event.results[index].isFinal,
          transcript: event.results[index][0].transcript,
        }))

        const { sessionFinal, interimChunk } = buildSessionTranscript(slices)
